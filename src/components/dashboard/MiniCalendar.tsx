    ...
    <motion.div variants={itemVariants} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          <h3 className="font-semibold text-white">{calData.currentMonth} {calData.currentYear}</h3>
          <button
            ...
            title={isFilteredByUser ? "Afficher toutes les tâches" : "Afficher uniquement mes tâches"}
          >
      ...
      <div className="grid grid-cols-7 gap-x-0 gap-y-1 mb-2">
        {weekdays.map((day, i) => (
          <div key={i} className="flex items-center justify-center h-8 text-xs font-medium text-zinc-400">
            {day}
          </div>
        ))}
      </div>
      ...
      {isPending ? (
        ...
        <>
          <div className="grid grid-cols-7 gap-x-0 gap-y-1">
            {days.map((day, index) => (
              day === null ? (
                <div key={`empty-${index}`} className="flex items-center justify-center h-8 w-full"></div>
              ) : (
                <div 
                  key={day}
          ...
          <div className="mt-6 space-y-3">
            ...
            <div className="space-y-2">
              {filteredEvents.length > 0 ? (
                (selectedDay ? allEventsForSelectedDay : filteredEvents).map((event: CalendarEvent) => (
                  <div key={event.id} className="flex items-center gap-3 p-2 rounded-lg bg-zinc-800/50 hover:bg-zinc-800 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between">
                        <p className="text-xs text-zinc-400">
                          {selectedDay ? event.timeRange : `${event.formattedDate}, ${event.timeRange}`}
                        </p>

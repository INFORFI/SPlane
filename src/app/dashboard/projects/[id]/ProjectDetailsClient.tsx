...
import { 
  ProjectWithDetails 
} from '@/action/projects/getProjectById';
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-sm font-medium ${
                      project.endDate && getDaysLeft(project.endDate) && getDaysLeft(project.endDate)! < 7 
                        ? 'text-rose-400' 
                        : 'text-white'
                    }`}>
                      {project.endDate 
            ...
            <div className="space-y-3">
              {project.teamMembers.length > 0 ? (
                project.teamMembers.map(member => (
                  <div key={member.id} className="flex items-center gap-3 p-2 hover:bg-zinc-800/50 rounded-lg transition-colors">
                    <div className="h-9 w-9 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-medium">
                      {member.fullName.split(' ').map(name => name[0]).join('')}
                    </div>
                    <div>
                  ...
                  <p className="text-sm text-zinc-600 mb-4 max-w-md mx-auto">
                    {activeTab !== 'all' 
                      ? `Il n'y a pas de tâches dans la catégorie "${
                          ...
                          activeTab === 'in-progress' ? 'En cours' : 'Terminées'
                        }".` 
                ...
                <>
                  <div className="flex gap-3">
                    <div className="h-8 w-8 rounded-full bg-emerald-600 flex items-center justify-center text-white text-sm font-medium">
                      {project.teamMembers[0]?.fullName.split(' ').map(name => name[0]).join('') || 'U'}
                    </div>
                    <div>
                      <p className="text-sm text-white">
                        <span className="font-medium">{project.teamMembers[0]?.fullName || 'Un utilisateur'}</span> a été assigné à la tâche <span className="font-medium">{project.tasks[0].title}</span>
                      </p>
                  ...
                  <div className="flex gap-3">
                    <div className="h-8 w-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-medium">
                      {project.owner.fullName.split(' ').map(name => name[0]).join('')}
                    </div>
                    <div>
                      <p className="text-sm text-white">
                        <span className="font-medium">{project.owner.fullName}</span> a créé le projet <span className="font-medium">{project.name}</span>
                      </p>
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2 items-center sm:justify-end">
          ...
          {/* Status badge */}
          <div className={`px-2.5 py-1 rounded-full text-xs font-medium ${status.bg} ${status.text}`}>
            {status.label}
          </div>
          
          {/* Assigned user */}
          {assignedUser ? (
            <div title={assignedUser.fullName} className="h-6 w-6 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs">
              {assignedUser.fullName.split(' ').map((name: string) => name[0]).join('')}
            </div>
          ) : (
            <div title="Non assignée" className="h-6 w-6 rounded-full bg-zinc-700 flex items-center justify-center text-zinc-500 text-xs border border-zinc-600">
              ?
            </div>
          )}

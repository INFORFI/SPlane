...
export default function PatchnoteDetail({ patchnote }: PatchnoteDetailProps) {
  ...
  const parsedContent =
    typeof patchnote.content === 'string' ? JSON.parse(patchnote.content) : patchnote.content;
  ...
  return (
    ...
    >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[var(--foreground-tertiary)]">
                      {itemsArray.length} élément{itemsArray.length > 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
      ...
      {Object.values(parsedContent).every(
        (items: unknown) => !items || (items as unknown[]).length === 0
      ) && (
        <motion.div
          variants={itemVariants}

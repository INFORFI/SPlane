...
export default function CalendarClient({ tasks }: CalendarClientProps) {
  ...
  const generateCalendarDays = () => {
    ...
    const prevMonthDays = [];
    if (firstDayIndex > 0) { // Skip if month starts on Sunday
  ...
  const getStatusInfo = (status: TaskStatus) => {
    switch (status) {
      case TaskStatus.COMPLETED:
        return { color: 'bg-emerald-500', textColor: 'text-emerald-400', bg: 'bg-emerald-500/10', label: 'Terminée' };
      case TaskStatus.IN_PROGRESS:
        return { color: 'bg-amber-500', textColor: 'text-amber-400', bg: 'bg-amber-500/10', label: 'En cours' };
      case TaskStatus.CANCELED:
        return { color: 'bg-zinc-500', textColor: 'text-zinc-400', bg: 'bg-zinc-500/10', label: 'Annulée' };
      default:
        return { color: 'bg-indigo-500', textColor: 'text-indigo-400', bg: 'bg-indigo-500/10', label: 'À faire' };
    }
  };
              ...
              <div className="flex rounded-md overflow-hidden border border-zinc-800">
                <button
                  ...
                  className={`px-3 py-1 text-xs font-medium ${
                    currentView === 'month' ? 'bg-indigo-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200'
                  } transition-colors`}
                >
                  Mois
                </button>
                <button
                  ...
                  className={`px-3 py-1 text-xs font-medium ${
                    currentView === 'week' ? 'bg-indigo-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200'
                  } transition-colors`}
                >
                  Semaine
                </button>
                <button
                  ...
                  className={`px-3 py-1 text-xs font-medium ${
                    currentView === 'day' ? 'bg-indigo-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200'
                  } transition-colors`}
                >
                  Jour
                </button>
              ...
              >
                ...
                {selectedDateTasks.length > 0 ? (
                    ...
                    {selectedDateTasks.map(task => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className={`h-2 w-2 rounded-full mt-2 ${getPriorityColor(task.priority)}`}></div>
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusInfo(task.status).bg} ${getStatusInfo(task.status).textColor}`}>
                          ...
                          {task.userTasks && task.userTasks[0] && (
                            ...
                            >
                              {task.userTasks[0].user.fullName.split(' ').map(n => n[0]).join('')}
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-zinc-500">
                    ...
                    <button
                      className="mt-3 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-sm font-medium text-white transition-colors flex items-center gap-2"
                    >
                      <Plus className="h-4 w-4" />
                  ...
                  return (
                    <div 
                      ...
                      className={`p-3 rounded-lg transition-colors ${
                        isUrgent ? 'bg-rose-500/10 border border-rose-500/20' : 'bg-zinc-800/50 hover:bg-zinc-800'
                      }`}
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <div className={`flex items-center gap-1 text-xs font-medium ${isUrgent ? 'text-rose-400' : 'text-zinc-400'}`}>
                      ...
                      <div className="flex justify-between items-center text-xs">
                        <div className={`px-2 py-1 rounded-full ${getStatusInfo(task.status).bg} ${getStatusInfo(task.status).textColor}`}>
                          {getStatusInfo(task.status).label}
                        </div>
                ...
                return (
                  <div key={projectId} className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs text-zinc-500">
                      <span>{completedTasks} sur {totalTasks} tâches</span>
                      {project.endDate && (
                        <span>
                          Échéance: {new Date(project.endDate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}
                        </span>
                      )}
                    </div>
                  </div>
